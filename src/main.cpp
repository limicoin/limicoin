...
const string strMessageMagic = "LimiCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
     ...
     {
         nSubsidy = 815000 * COIN;			// ~ 2% total coins
     }
     else if(pindexBest->nHeight < 180)
     {
         nSubsidy = 1 * COIN;									//total of 180 coins
     }
     else if(pindexBest->nHeight < 1440) //to the end of day 1
     {
         nSubsidy = 3000 * COIN;								//3780000 coins
     }
     else if(pindexBest->nHeight < 2880) //to the end of day 2
     {
         nSubsidy = 4500 * COIN;								//6480000
     }
     else if(pindexBest->nHeight < 4320) //to the end of day 3
     {
         nSubsidy = 6000 * COIN;								//8640000
     }    
     else if(pindexBest->nHeight < 5760) //to the end of day 4
     {
         nSubsidy = (pindexBest->nHeight + 1000) * COIN;		//7256880
     }
     else if(pindexBest->nHeight < 7200) //to the end of day 5
     {
         nSubsidy = 6000 * COIN;								//8640000
     }
     else if(pindexBest->nHeight < 8640) //to the end of day 6
     {
         nSubsidy = 3000 * COIN;								//3780000
     }
	 //to block 10000	(1360 blocks)					 day 7	//1360000
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "LimiCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Built 30/07/2014 - MiliCoin";
        CTransaction txNew;
        txNew.nTime = 1406678400;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xdf39d1aaba5e36f9020ac9f026a4c1afceefc72f839a27cfff6b2d3231162f8f"));
        block.print();
